...
#' @export
geopressuretemplate_tag_mag <- function(
    id,
  ...
  # Calibrate magnetic data if available
  ...
  # If magnetic mapping is required, process it
  if ("map_magnetic" %in% config$geopressuretemplate$likelihood) {
    if (!quiet) {
      cli::cli_h2("Build magnetic likelihood map")
    }
